...
class InternalPmCacheCfgUtil {
    ...
    for (Observe clearOn : clearOns) {
      for (String expression : clearOn.pm()) {
        ...
        if (clearOn.observePmTree()) {
          registerClearOnListenerRecursive(pmToObserve, e);
        } else {
          PmEventApi.addPmEventListener(pmToObserve, PmEvent.VALUE_CHANGE, e); 
        }
      }
    }
  } 
  ...
  private static void addToListAndAssertNoMixedMode(PmObjectBase pm, Annotation cfg, List foundAnnotations) {
    if (!foundAnnotations.isEmpty()) {
      ...
      if (PmCacheCfg.class.isAssignableFrom(a.getClass()) && !(PmCacheCfg.class.isAssignableFrom(cfg.getClass()))) {
        throw new IllegalStateException("Mixed cache annotations are not supported. We are currently searching for '"+PmCacheCfg.class+"' annotaions but pm '"+pm.getPmRelativeName()+"' is annotated with '"+PmCacheCfg2.class+"'. Please make sure that you either use the old cache cfg annotation or the new one, but not both!");        
      }
      if (PmCacheCfg2.class.isAssignableFrom(a.getClass()) && !(PmCacheCfg2.class.isAssignableFrom(cfg.getClass()))) {
        throw new IllegalStateException("Mixed cache annotations are not supported. We are currently searching for '"+PmCacheCfg2.class+"' annotaions but pm '"+pm.getPmRelativeName()+"' is annotated with '"+PmCacheCfg.class+"'. Please make sure that you either use the old cache cfg annotation or the new one, but not both!");        
      }
    }
  ...
  /**
   * Recursively registers the given listener to all PMs of the given hierarchy.
   * 
   * @param rootPm
   * @param e
   */
  private static void registerClearOnListenerRecursive(PmObject rootPm, PmEventListener e) {
    PmEventApi.addPmEventListener(rootPm, PmEvent.VALUE_CHANGE, e);
    List<PmObject> children = ((PmObjectBase) rootPm).getPmChildren();
    if (children != null && !children.isEmpty()) {
      for (PmObject child : children) {
        registerClearOnListenerRecursive(child, e);
      }
    }
  }
