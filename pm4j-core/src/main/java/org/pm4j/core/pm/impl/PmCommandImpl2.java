 ...
 *
 * TODO: will be published as the new command version in the next refactoring.
 * The old one has some logical issues (especially enablement) an will be deprecated.
 *
 * @author Olaf Boede
 */
class PmCommandImpl2 extends PmObjectBase implements PmCommand, Cloneable {

  private static final Logger LOG = LoggerFactory.getLogger(PmCommandImpl2.class);
   ...
   */
  private PmCommandImpl2 templateCommand;
   ...
   */
  public PmCommandImpl2(PmObject pmParent, NaviLink naviLink) {
    this(pmParent);
   ...
   *          The presentation model that command acts on.
   */
  public PmCommandImpl2(PmObject pmParent) {
    super(pmParent);
  }
  ...
  @Override
  public PmCommandImpl2 clone() {
    PmCommandImpl2 clone = (PmCommandImpl2)super.clone();
  ...
  protected boolean beforeDo() {
    if (!isPmEnabled()) {
      // XXX: check exception usage here.
  ...
  // XXX: check if it can be moved to a view connector.
  ...
  protected void initMetaData(PmObjectBase.MetaData metaData) {
    ...
    if (annotation != null) {
      // XXX: optimize memory size for default case.
      myMetaData.beforeDo = new LinkedHashSet<BEFORE_DO>(Arrays.asList(annotation.beforeDo()));
      myMetaData.afterDo = new LinkedHashSet<AFTER_DO>(Arrays.asList(annotation.afterDo()));
      ...
      myMetaData.beforeDo = new HashSet<BEFORE_DO>(Arrays.asList(BEFORE_DO.DEFAULT));
      myMetaData.afterDo = new HashSet<AFTER_DO>(Arrays.asList(AFTER_DO.DEFAULT));
    }
    ...
    if (myMetaData.beforeDo.contains(BEFORE_DO.DEFAULT)) {
      ...
      myMetaData.beforeDo = new HashSet<BEFORE_DO>(Arrays.asList(BEFORE_DO.VALIDATE));
    }
