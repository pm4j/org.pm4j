...
/**
 * A {@link PageableCollection} instance that provides {@link PmBean} instances in
 * front of a {@link PageableCollection} container that handles the corresponding
 * bean instances.
 *
 * @author olaf boede
 *
 ...
 */
public class PmBeanCollection<T_PM extends PmBean<T_BEAN>, T_BEAN> extends PropertyChangeSupportedBase implements PageableCollection<T_PM> {

  /** The collection type specific selection handler. */
  private PmBeanSelectionHandler<T_PM, T_BEAN> selectionHandler;
  private final PmBeanCollectionModificationHandler   modificationHandler;

  private final PmObject                              pmCtxt;
  private final PageableCollection<T_BEAN>           beanCollection;
  ...
  public PmBeanCollection(final PmObject pmCtxt, Class<?> itemPmClass, PageableCollection<T_BEAN> pageableBeanCollection) {
    assert pmCtxt != null;
    ...
    this.selectionHandler = new PmBeanSelectionHandler<T_PM, T_BEAN>(pmCtxt, pageableBeanCollection.getSelectionHandler());
    ...
    beanCollection.addPropertyChangeListener(PageableCollection.EVENT_ITEM_ADD, new PropertyChangeListener() {
      @Override
    ...
    beanCollection.addPropertyChangeListener(PageableCollection.EVENT_ITEM_UPDATE, new PropertyChangeListener() {
      @Override
      public void propertyChange(PropertyChangeEvent evt) {
    ...
    beanCollection.addPropertyChangeListener(PageableCollection.EVENT_REMOVE_SELECTION, new PropertyChangeListener() {
      @Override
      public void propertyChange(PropertyChangeEvent evt) {
    ...
    beanCollection.addPropertyChangeListener(PageableCollection.PROP_PAGE_IDX, new PropertyChangeListener() {
      @Override
      public void propertyChange(PropertyChangeEvent evt) {
    ...
    beanCollection.addPropertyChangeListener(PageableCollection.PROP_PAGE_SIZE, new PropertyChangeListener() {
      @Override
      public void propertyChange(PropertyChangeEvent evt) {
   ...
   */
  public PmBeanCollection(PmObject pmCtxt, Class<T_PM> itemPmClass, Collection<T_BEAN> beans, QueryOptions queryOptions) {
    this(pmCtxt, itemPmClass,
         new InMemCollectionImpl<T_BEAN>(
             beans,
             queryOptions)
        );
  }
  ...
  /**
   * Creates a collection backed by a {@link InMemCollectionImpl} with no query constraints.
   *
   * @param pmCtxt
   *          The PM context used to create the PM's for the bean items.
   * @param beans
   ...
   */
  public PmBeanCollection(PmObject pmCtxt, Class<T_PM> itemPmClass, Collection<T_BEAN> beans) {
   ...
   */
  public PageableCollection<T_BEAN> getBeanCollection() {
   ...
   */
  protected void setSelectionHandler(PmBeanSelectionHandler<T_PM, T_BEAN> selectionHandler) {
  ...
  class PmBeanCollectionModificationHandler implements ModificationHandler<T_PM> {
    ...
    @Override
    public void registerUpdatedItem(T_PM item, boolean isUpdated) {
      getBeanCollectionModificationHandler().registerUpdatedItem(item.getPmBean(), isUpdated);
    }
    ...
    public boolean removeSelectedItems() {
      ...
      try {
        PmBeanCollection.this.fireVetoableChange(PageableCollection.EVENT_REMOVE_SELECTION, selection, null);
    ...
    class PmBeanModifications implements Modifications<T_PM> {
      ...
      public Selection<T_PM> getRemovedItems() {
        return new PmBeanSelection<T_PM, T_BEAN>(pmCtxt, getBeanCollectionModificationHandler().getModifications().getRemovedItems());
      }
    };
