...
public class PmTableSortTest {
  ...
  public void testTableWithInitialSortOrder() {
    Comparator<Item> idxComparator = new Comparator<Item>() {
      @Override public int compare(Item o1, Item o2) { return CompareUtil.compare(o1.idx, o2.idx); }
    };
    myTablePm.setPageableCollection(new PageablePmsForBeans<ItemPm, Item>(myTablePm, rowBeanList, idxComparator), false);

    assertEquals("The table sorts by bean attribute 'idx'.", 1, myTablePm.getRows().get(0).getPmBean().idx);
    assertEquals("The table sorts by bean attribute 'idx'.", 2, myTablePm.getRows().get(1).getPmBean().idx);
    assertEquals("The table sorts by bean attribute 'idx'.", 3, myTablePm.getRows().get(2).getPmBean().idx);

    myTablePm.name.getSortOrderAttr().setValue(PmSortOrder.DESC);
    assertEquals("The table now sorts by name in descending order.", "c", myTablePm.getRows().get(0).name.getValue());
    assertEquals("The table now sorts by name in descending order.", "b", myTablePm.getRows().get(1).name.getValue());
    assertEquals("The table now sorts by name in descending order.", "a", myTablePm.getRows().get(2).name.getValue());

    myTablePm.name.getSortOrderAttr().setValue(PmSortOrder.NEUTRAL);
    assertEquals("The table sorts again by bean attribute 'idx'.", 1, myTablePm.getRows().get(0).getPmBean().idx);
    assertEquals("The table sorts again by bean attribute 'idx'.", 2, myTablePm.getRows().get(1).getPmBean().idx);
    assertEquals("The table sorts again by bean attribute 'idx'.", 3, myTablePm.getRows().get(2).getPmBean().idx);
  }
