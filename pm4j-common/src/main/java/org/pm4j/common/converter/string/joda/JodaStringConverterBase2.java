...
import org.pm4j.common.converter.string.StringConverter2;
 ...
 * @author oboede
 */
public abstract class JodaStringConverterBase2<T> implements StringConverter2<T> {
  ...
  private MultiFormatParserBase<T> multiFormatParser = new MultiFormatParserBase<T>() {
   // @Override
    protected T parseValue(String input, String format, Locale locale, StringConverterCtxt ctxt) throws ParseException {
  ...
  @Override
  public String valueToString(StringConverterCtxt ctxt, T v) {
    String outputFormat = multiFormatParser.getOutputFormat(ctxt);
