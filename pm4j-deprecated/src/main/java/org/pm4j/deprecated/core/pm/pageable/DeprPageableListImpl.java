...
/**
 * Implements a {@link DeprPageableCollection} based on an {@link List} of items to
 * handle.
 *
 * @author olaf boede
 *
 ...
 *
 * @deprecated please use {@link InMemCollectionImpl}
 */
@Deprecated
public class DeprPageableListImpl<T_ITEM> implements DeprPageableCollection<T_ITEM> {
  ...
  private DeprFilter             currentFilter;
   ...
   */
  public DeprPageableListImpl(Collection<T_ITEM> objects, Comparator<T_ITEM> initialSortComparator) {
   ...
   *          The set of objects to iterate over.
   */
  public DeprPageableListImpl(Collection<T_ITEM> objects) {
    this(objects, null);
  }
  ...
  public DeprPageableListImpl(int pageSize, boolean multiSelect) {
  ...
  public List<T_ITEM> getItemsOnPage() {
    ...
    int first = DeprPageableCollectionUtil.getIdxOfFirstItemOnPage(this) - 1;
    int last = DeprPageableCollectionUtil.getIdxOfLastItemOnPage(this);
  ...
  private void _applyFilterAndSortOrder() {
    ...
    if (currentPageIdx * pageSize >= filteredList.size()) {
      currentPageIdx = DeprPageableCollectionUtil.getNumOfPages(this);
    }
